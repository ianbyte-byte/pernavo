...
- **Teammates**: Independent agents with their own context windows. Can read `~/.claude/teams/{team-name}/config.json` for discovery.
- **Shared task list**: Use it to assign and track work. Teammates should self-claim the next unassigned, unblocked task when idle. Aim for 5-6 tasks per teammate.
...
- **Plan Approval**: For complex or risky tasks (e.g., refactors), the lead should spawn teammates with `Require plan approval before they make any changes`. The lead reviews and approves/rejects plans autonomously (no 'TODO' markers).
- **Display Mode**: Configure `teammateMode` (auto/in-process/tmux) in `~/.claude/settings.json`.
- **Communication**:
...
- **Cleanup**: MANDATORY SEQUENCE: The lead must shut down all teammates first, wait for confirmation, and *then* run `Clean up the team`.
