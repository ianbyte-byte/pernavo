...
- **Teammates**: Independent agents with their own context windows. Inherit lead's permissions and model by default.
...
- **UI Shortcuts**: Use `Up/Down` or `Shift+Down` to cycle through teammates in the agent panel, `Ctrl+T` to toggle the task list, `Enter` to view a teammate's transcript, and `Escape` to interrupt.
- **Display Modes**: Set via `teammateMode` in `settings.json` or `--teammate-mode` flag.
  - `in-process`: (Default) All teammates run in the main terminal.
  - `auto`/`tmux`: Split panes using tmux or iTerm2.
...
- **Cleanup**: Since v2.1.178, cleanup of shared team directories happens automatically when the session exits.
- **Parallel patterns**:
