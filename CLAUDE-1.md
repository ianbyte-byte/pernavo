...
- **Team lead**: The main agent session. Responsible for spawning the team, approving plans, monitoring progress, and final synthesis.
- **Teammates**: Independent agents with their own context windows. You can reuse subagent definitions (e.g., `lcc-security-reviewer`) as teammates by referencing their name when spawning.
- **Display Modes**:
  - `in-process`: Default. Teammates run inside the lead's terminal.
  - `split-panes`: Teammates run in separate panes (requires `tmux` or iTerm2). Configure via `teammateMode` in `settings.json`.
...
- **Cleanup**: The lead must shut down all teammates FIRST (e.g., "Ask the team to shut down"), then run `Clean up the team`.
- **Monitoring & Steering**: Check in on teammates, redirect if needed, and synthesize findings as they arrive.
- **Parallel patterns**:
...
- **Limitations**:
  ...
  - Task lag: Teammates may fail to mark tasks complete. Update manually or nudge if stuck.
  - One team per session; no nested teams; fixed lead role.
