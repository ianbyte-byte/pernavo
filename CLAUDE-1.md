...
- **UI Shortcuts**: Use `Up/Down` or `Shift+Down` to cycle through teammates, `Ctrl+T` to toggle the task list, `Enter` to view a teammate's session, and `Escape` to interrupt.
...
- **Display Modes**:
  - `in-process`: Default, teammates run within the main terminal.
  - `split-panes`: Requires tmux or iTerm2. Set via `teammateMode: auto` in `.claude/settings.json`.
- **Cleanup**: Team cleanup happens automatically upon session exit. Graceful shutdown of teammates by name is recommended. The `TeamCreate` and `TeamDelete` tools are deprecated and no longer used.
- **Parallel patterns**:
