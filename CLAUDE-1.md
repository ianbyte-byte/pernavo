...
- **Teammates**: Independent agents with their own context windows. They load project context (CLAUDE.md, skills) but do not inherit the Lead's conversation history.
- **Shared task list**: Use it to assign and track work. Teammates can self-claim tasks. Aim for 5-6 tasks per teammate. Use `depends on` for task dependencies.
...
- **Synchronization**: Use "Wait for your teammates to complete their tasks before proceeding" to ensure the Lead waits for completion before synthesis.
- **Cleanup**: Once the task is complete, ask teammates to shut down and then run `Clean up the team`.
- **Parallel patterns**:
