...
- **Teammates**: Independent agents with their own context windows. Discover other members via `~/.claude/teams/{team-name}/config.json`.
- **Shared task list**: Use it to assign and track work. Teammates can self-claim tasks. Aim for 5-6 tasks per teammate to maximize productivity. Subjects < 10 characters or with "TODO" are rejected by `TaskCreated` hook.
...
- **Cleanup Sequence**: (1) Wait for all teammates to finish, (2) Explicitly ask them to shut down, (3) Final Synthesis, (4) `Clean up the team`.
- **Parallel patterns**:
...
- **TaskCreated**: Fires before a task is created. Rejects subjects < 10 chars or with "TODO".
