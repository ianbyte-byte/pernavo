...
- **Teammates**: Independent agents with their own context windows. Inherit lead's model and effort level by default (v2.1.186+).
...
- **UI Shortcuts**:
  - `Up/Down` (in-process): Select teammate in the agent panel.
  - `Enter`: View the selected teammate's transcript or open their pane.
  - `x`: Stop the selected teammate.
  - `Ctrl+T`: Toggle the shared task list.
  - `Escape`: Interrupt the current turn.
- **Display Modes**: Configure `teammateMode` in `settings.json` (`in-process`, `auto`, `tmux`, `iterm2`).
- **Plan Approval**: For complex or risky tasks, spawn teammates with `Require plan approval before they make any changes`. The teammate stays in read-only mode until the lead approves their plan.
- **Communication**:
  - `message <teammate>`: Send a direct message to a specific teammate.
  - `broadcast <message>`: Send to all teammates.
- **Cleanup**: Cleanup happens automatically when the session exits. To end a teammate early, use `Ask the [name] teammate to shut down`. The `TeamCreate` and `TeamDelete` tools are deprecated.
- **Parallel patterns**:
