...
- **Teammates**: Independent agents with their own context windows. Spawning follows the pattern: `Spawn a teammate using the [name] agent type`.
...
- **UI Shortcuts**: Use `Up/Down` arrows to cycle through teammates in the agent panel, `Enter` to view/message a teammate's session, `x` to stop a selected teammate, `Ctrl+T` to toggle the task list, and `Escape` to interrupt.
...
- **Cleanup**: Handled automatically when the session exits. No manual `Clean up the team` step is required (v2.1.178+).
- **Parallel patterns**:
