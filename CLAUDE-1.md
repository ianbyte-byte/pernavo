...
- **Team lead**: The main agent session. Coordinates work, assigns tasks, approves plans, and synthesizes results.
- **Teammates**: Independent agents with their own context windows. They do NOT inherit the lead's conversation history; provide rich context in the spawn prompt.
- **Shared task list**: Use it to coordinate work. Teammates can self-claim tasks. Aim for 5-6 tasks per teammate to maximize productivity.
- **UI Shortcuts**: `Shift+Down` (cycle), `Ctrl+T` (toggle task list), `Enter` (view teammate), `Escape` (interrupt).
- **Plan Approval**: For complex/risky tasks, spawn teammates with `Require plan approval before they make any changes`. The lead reviews plans autonomously (e.g., "approve only if it includes tests").
- **Communication**:
  - `message <teammate>`: Direct message (e.g., Coder to Reviewer).
  - `broadcast <message>`: Team-wide message (use sparingly).
- **Orchestration**:
  - **Wait for completion**: The lead should wait for teammates to finish their tasks before proceeding with its own implementation or synthesis.
  - **Cleanup**: Once complete, the lead must shut down all teammates first, then run `Clean up the team`.
- **Parallel patterns**:
  - **Scientific Debate**: 5+ teammates investigating competing hypotheses and actively disproving each other.
  - **Parallel Review**: Reviewers with distinct lenses (Security, Performance, Test Coverage).
