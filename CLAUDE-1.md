...
- **Teammates**: Independent agents with their own context windows. They load the project context (CLAUDE.md, skills, etc.) but NOT the lead's conversation history.
- **Teammate Discovery**: Teammates can discover each other by reading `~/.claude/teams/{team-name}/config.json`.
- **Shared task list**: Use it to assign and track work. Teammates can self-claim tasks. Aim for 5-6 tasks per teammate to maximize productivity. Tasks can have dependencies that unblock automatically.
...
- **Plan Approval**: For complex or risky tasks (e.g., refactors), the lead should spawn teammates with `Require plan approval before they make any changes`. The lead reviews and approves/rejects plans autonomously based on provided criteria.
- **Communication**:
...
- **Cleanup**: Once the task is complete, the lead must shut down all teammates first and then run `Clean up the team` to remove shared resources. Cleanup will fail if any teammate is still active.
- **Parallel patterns**:
