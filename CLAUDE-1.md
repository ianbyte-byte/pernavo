...
- **UI Shortcuts**: Use `Up/Down` arrows in the agent panel to cycle through teammates, `x` to stop a selected teammate, `Ctrl+T` to toggle the task list, `Enter` to view a teammate's session, and `Escape` to interrupt.
...
- **Communication**:
  - **Context Window**: Teammates load project context (`CLAUDE.md`, skills) but do **not** inherit the lead's conversation history. Provide rich context in the spawn prompt.
...
- **Cleanup**: Teammates should be asked to shut down when finished. Shared resources and team configuration are cleaned up automatically when the lead session ends.
- **Parallel patterns**:
