...
- **teammateMode**: "auto" (default, favors split-panes in tmux/iTerm2) or "in-process".
...
- **Plan Approval**: For complex or risky tasks (e.g., refactors), the lead should spawn teammates with `Require plan approval before they make any changes`. The lead reviews and approves/rejects plans autonomously based on criteria (e.g., test coverage, no breaking changes).
- **Communication**:
...
- **Shutdown & Cleanup**:
  ...
  - Once all teammates are shut down, the lead must run `Clean up the team`.
- **Limitations**: No session resumption for in-process teammates; task status can lag (update manually if stuck).
- **Parallel patterns**:
