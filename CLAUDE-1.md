...
- **Teammates**: Independent agents with their own context windows. They do not inherit the lead's history.
- **Spawning**: Use `Spawn a teammate using the [agent-type] agent type`. No setup step needed.
...
- **Cleanup**: Cleanup happens automatically when the session exits. No manual `Clean up the team` or `TeamDelete` tool call is required.
- **Parallel patterns**:
