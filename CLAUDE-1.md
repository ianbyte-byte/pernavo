...
- **Plan Approval**: For complex or risky tasks (e.g., refactors), the lead should spawn teammates with `Require plan approval before they make any changes`.
  - **Autonomous Review**: The lead reviews and approves/rejects plans. Plans with 'TODO' markers or lacking tests should be rejected.
- **Communication**:
...
- **Settings**:
  - `teammateMode`: Set in `.claude/settings.json` (defaults to `in-process`). Use `split-panes` for tmux/iTerm2.
