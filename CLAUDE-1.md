...
- **UI Shortcuts**: Use `Up/Down` arrows to select a teammate in the agent panel, `Enter` to view a teammate's session, `Escape` to interrupt a turn, and `Ctrl+T` to toggle the task list.
...
- **Cleanup**: Automatic cleanup happens when the session ends. To end a teammate session early, ask them to shut down by name (e.g., `Ask the researcher teammate to shut down`).
- **Parallel patterns**:
