...
Agent teams allow parallel execution and decentralized coordination. Set `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` to enable.

- **Display mode**: Configure `teammateMode` in `~/.claude/settings.json` or via `--teammate-mode`. Options: `in-process` (default), `tmux` (split panes, requires tmux/iTerm2), `auto`.
...
- **Shared task list**: Use it to assign and track work. Teammates can self-claim tasks. Aim for 5-6 tasks per teammate to maximize productivity. Tasks can have dependencies.
...
- **Communication**:
  ...
  - Teammates can read `~/.claude/teams/{team-name}/config.json` to discover other members.
...
- **Parallel patterns**:
  - **Scientific Debate**: Spawn 5+ teammates to investigate competing hypotheses and actively disprove each other. Use `Sonnet` for teammates for optimal results.
