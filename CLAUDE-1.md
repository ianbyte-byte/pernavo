...
- **UI Shortcuts & Control**:
  - `Shift+Down`: Cycle through teammates in in-process mode.
  - `Ctrl+T`: Toggle the task list.
  - `Enter`: View a teammate's session.
  - `Escape`: Interrupt a teammate's current turn.
  - `teammateMode`: Set to `auto` (default), `in-process`, or `tmux` in `.claude/settings.json`. Split panes require `tmux` or iTerm2.
...
- **Communication**:
  ...
  - Teammates inherit the lead's permission settings but load skills/MCP from project settings (not subagent frontmatter).
- **Cleanup**: Once the task is complete, the lead must shut down all teammates and then run `Clean up the team` to remove shared resources. Only one team can be active per session.
- **Parallel patterns**:
...
- **Limitations**:
  ...
  - One team at a time; clean up before starting another.
  - Shutdown Finishing: Teammates finish their current request before exiting.
