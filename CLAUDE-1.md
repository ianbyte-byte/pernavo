...
- **Display Modes**:
  - `in-process`: All teammates run in the main terminal. Use `Shift+Down` to cycle.
  - `split panes`: Requires `tmux` or iTerm2. Each teammate gets a pane.
  - Configuration: Set `teammateMode` in `~/.claude.json` or use `--teammate-mode` flag.
- **Shared task list**: Use it to assign and track work. Teammates can self-claim tasks.
  - **Task Sizing**: Aim for 5-6 tasks per teammate. Keep tasks small enough for clear deliverables but large enough to justify coordination.
...
- **Communication**:
  ...
  - `broadcast <message>`: Send to all teammates (use sparingly, high token cost).
- **Cleanup**: Once the task is complete, the lead must shut down all teammates (one by one if needed) and then run `Clean up the team` to remove shared resources.
- **Token Usage**: Be mindful that each teammate is a separate instance with its own context window; costs scale with team size.
- **Parallel patterns**:
...
- **TaskCreated**: Fires when a task is added. Used to enforce descriptive subjects (>= 10 chars, no "TODO").
- **TaskCompleted**: Fires when a task is closed. Used to verify completion criteria and "TODO" absence.
