...
- **Teammates**: Independent agents with their own context windows. They can discover each other via `~/.claude/teams/{team-name}/config.json`.
...
- **Coordination**: If the lead starts working prematurely, use `Wait for your teammates to complete their tasks before proceeding`.
- **Cleanup**: Once the task is complete, the lead must ask all teammates to shut down, wait for them to finish, and then run `Clean up the team` to remove shared resources.
- **Limitations**: Agent teams are experimental. `/resume` and `/rewind` do not restore in-process teammates.
- **Parallel patterns**:
