...
- **Display Modes**:
  - `auto`: Uses split panes if in tmux or iTerm2, otherwise in-process.
  - `in-process`: All teammates run inside the main terminal.
  - `split panes`: Each teammate gets its own pane (requires tmux or iTerm2).
- **Shared task list**: Use it to assign and track work. Teammates should self-claim available work. Aim for 5-6 tasks per teammate.
...
- **Plan Approval**: Use `Require plan approval before they make any changes` for complex/risky tasks. Teammates work in read-only mode until the lead approves.
- **Communication**:
  - `message <teammate>`: Send a direct message to a specific teammate.
  - `broadcast <message>`: Send to all teammates (use sparingly).
- **Coordination**: Use "Wait for your teammates to complete their tasks before proceeding" to keep the lead from acting too early.
