...
- **Display Modes**:
  - `in-process`: Default. Teammates run inside the main terminal.
  - `split panes`: Each teammate in its own pane. Requires `tmux` or iTerm2 (`it2` CLI).
  - Config: Set `teammateMode` in `~/.claude.json`.
...
- **Discovery**: Teammates can discover other members by reading `~/.claude/teams/{team-name}/config.json`.
- **Communication**:
...
- **Limitations**:
  ...
  - **Task status lag**: Teammates may fail to mark tasks as completed; nudge them if stuck.
  - **Shutdown behavior**: Teammates finish their current request before shutting down.

## 6) Hooks and quality gates
Automated checks are enforced via `.claude/settings.json` and `.claude/hooks/`.
- **TaskCreated**: Runs when a task is created. Used to validate subjects (e.g., minimum length, no "TODO").
...
- **Primary Doc Index**: `.claude/docs/claud_platform_menu.md`.
