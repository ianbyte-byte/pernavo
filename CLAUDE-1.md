...
- **Teammates**: Independent agents with their own context windows. Teammates discover each other via `~/.claude/teams/{team-name}/config.json`.
...
- **Cleanup Sequence**: 1) Wait for completion, 2) Synthesis, 3) Shut down teammates (one by one), 4) Run `Clean up the team`.
- **Parallel patterns**:
  - **Scientific Debate**: Spawn 5+ teammates to investigate competing hypotheses and actively disprove each other using `message`.
...
- **Context Preservation**: Spawn prompts must be rich and self-contained because teammates do not inherit the lead's conversation history.
