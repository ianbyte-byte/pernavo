...
if [[ "$EVENT" == "TaskCreated" ]]; then
  ...
  if [[ ${#TASK_SUBJECT} -lt 10 ]]; then
     echo "Task subject is too short ($TASK_SUBJECT). Please provide a descriptive subject (at least 10 characters)." >&2
     exit 2
  fi

  # Reject TODOs in task creation
  if [[ "$TASK_SUBJECT" == *"TODO"* ]]; then
     echo "Task subject contains TODO. Please provide a concrete, actionable subject." >&2
     exit 2
  fi
fi
...
if [[ "$EVENT" == "TaskCompleted" ]]; then
  ...
  # Final check: ensure subject didn't become a TODO during execution
  if [[ "$TASK_SUBJECT" == *"TODO"* ]]; then
  ...
  if [[ -f "$TRANSCRIPT_PATH" ]]; then
    if ! grep -qiE "handoff|summary|report|LGTM|verified|completed|finished" "$TRANSCRIPT_PATH"; then
       echo "Task completion requires evidence of completion (summary, report, LGTM, etc.) in the transcript." >&2
       exit 2
    fi
  fi
fi
