...
if [[ "$EVENT" == "TaskCreated" ]]; then
  ...
  # Prevent vague tasks or TODOs in subject
  if [[ "$TASK_SUBJECT" == *"TODO"* ]] || [[ ${#TASK_SUBJECT} -lt 10 ]]; then
     echo "Task subject is too short or contains TODO. Please provide a descriptive subject." >&2
     exit 2
  fi
fi
...
if [[ "$EVENT" == "TaskCompleted" ]]; then
  ...
  if [[ -f "$TRANSCRIPT_PATH" ]]; then
    if ! grep -qiE "handoff|summary|report|LGTM|verified|completed|finished" "$TRANSCRIPT_PATH"; then
       echo "Task completion requires a summary, handoff report, or completion confirmation (e.g., 'completed', 'finished', 'LGTM') in the transcript." >&2
       exit 2
    fi
  fi
fi
...
if [[ "$EVENT" == "TeammateIdle" ]]; then
  ...
  if [[ -f "$TRANSCRIPT_PATH" ]]; then
    if grep -qi "error" "$TRANSCRIPT_PATH" && ! grep -qiE "fixed|resolved|workaround|handled" "$TRANSCRIPT_PATH"; then
