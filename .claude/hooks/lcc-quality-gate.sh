...
if [[ "$EVENT" == "TaskCreated" ]]; then
  ...
  # Ensure task subject is descriptive and not just "TODO"
  ...
  if [[ ${#TASK_SUBJECT} -lt 10 ]]; then
     echo "Task subject is too short ($TASK_SUBJECT). Please provide a more descriptive subject (min 10 chars)." >&2
     exit 2
  fi
fi
...
if [[ "$EVENT" == "TaskCompleted" ]]; then
  ...
  # Double check: If it's a coder task, ensure it doesn't say "TODO" (though TaskCreated should catch it)
  if [[ "$TASK_SUBJECT" == *"TODO"* ]]; then
