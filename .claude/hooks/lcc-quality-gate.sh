...
if [[ "$EVENT" == "TaskCreated" ]]; then
  ...
  # Ensure task subjects are descriptive
  if [[ ${#TASK_SUBJECT} -lt 10 ]]; then
     echo "Task subject is too short (min 10 characters). Please be more descriptive." >&2
     exit 2
  fi
...
if [[ "$EVENT" == "TaskCompleted" ]]; then
  ...
  if [[ -f "$TRANSCRIPT_PATH" ]]; then
    if ! grep -qiE "handoff|summary|report|LGTM|verified|completed|finished" "$TRANSCRIPT_PATH"; then
       echo "Task completion requires a summary, handoff report, or completion keyword (LGTM, verified, completed, finished) in the transcript." >&2
       exit 2
    fi
  fi
fi
