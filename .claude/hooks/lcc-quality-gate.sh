...
if [[ "$EVENT" == "TaskCompleted" ]]; then
  ...
  # Verification: Ensure there is a handoff or a report in the transcript
  if [[ -f "$TRANSCRIPT_PATH" ]]; then
    if ! grep -qiE "handoff|summary|report|LGTM|verified" "$TRANSCRIPT_PATH"; then
       echo "Quality Gate: Task completion requires a summary or handoff report in the transcript." >&2
       exit 2
    fi
  fi
fi
