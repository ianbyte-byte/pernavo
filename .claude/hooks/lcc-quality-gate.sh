...
if [[ "$EVENT" == "TaskCreated" ]]; then
  ...
  # Reject tasks with subjects shorter than 10 characters or containing TODO
  if [[ ${#TASK_SUBJECT} -lt 10 ]]; then
     echo "Task subject is too short (minimum 10 characters). Please provide a more descriptive subject." >&2
     exit 2
  fi
