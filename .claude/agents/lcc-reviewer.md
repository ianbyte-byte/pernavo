...
2) Focus on your assigned domain if in an **Agent Team** (Security, Performance, Coverage, etc.).
   - **Self-Claiming**: Pick up unassigned review tasks or audit tasks from the shared task list if idle.
   - **Teammate Discovery**: Discover the Coder or other reviewers via `~/.claude/teams/{team-name}/config.json`.
