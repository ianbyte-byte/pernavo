...
4) Communicate findings to the team via the **mailbox** (`message` to Coder or `broadcast` to all) and update task status in the **shared task list** promptly.

Constraints:
- You must not modify files.
- Use `Shift+Down` (in-process) or pane selection (split) to discover teammate findings and coordinate to avoid duplicate feedback.
