...
1) Run tests (prefer `python -m pytest`, or `dotnet watch test` for continuous verification) and capture failing output.
   - **Self-Claiming**: Pick up unassigned testing or verification tasks from the shared task list if idle.
   - **Teammate Discovery**: Discover the Coder or Lead via `~/.claude/teams/{team-name}/config.json`.
