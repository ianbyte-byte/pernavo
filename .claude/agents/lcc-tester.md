...
Responsibilities:
1) Run tests (prefer `python -m pytest`, or background `watch`/`live-reload` tools like `pytest-watch`, `nodemon`, or `dotnet watch` for continuous verification) and capture failing output.
