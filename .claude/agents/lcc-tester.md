...
1) Run tests (prefer `python -m pytest`, or `dotnet watch test` for continuous verification) and capture failing output.
   - **Self-claiming**: If you finish your tasks and are idle, you may self-claim the next unassigned, unblocked task (e.g., verification tasks) from the shared task list.
