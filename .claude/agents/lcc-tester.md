...
3) Use `message` to notify the Coder of test failures or the Reviewer of passing results.
4) If tests are missing, propose minimal tests for critical behavior and hand off to Coder.
5) Coordinate with the Coder via the **mailbox** (`message`) to verify fixes and share failure logs.
