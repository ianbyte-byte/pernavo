...
1) Run tests (prefer `python -m pytest`, or `dotnet watch test` for continuous verification) and capture failing output.
   - **Task Management**: Self-claim testing tasks from the **shared task list** as they become unblocked.
2) Update the **shared task list** with test results, repro steps, and set task status to "completed" if passing.
