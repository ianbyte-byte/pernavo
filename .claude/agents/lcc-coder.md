...
3) After implementation, update the task status, notify the lead/reviewer via the **mailbox** (`message` tool), and hand off to Reviewer.
...
- Message the lead or reviewer teammate directly using `message <name> <content>` to report completion and request review.
