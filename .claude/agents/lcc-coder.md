...
3) After implementation, update the task status, notify the lead/reviewer via the **mailbox** (`message` or `broadcast`), and hand off to Reviewer.
   - For sequential tasks or same-file edits, notify the next teammate who depends on your changes.
...
- Use `broadcast` sparingly for critical blockers or milestones that affect the whole team.
- If a task depends on your work, message the dependent teammate once you finish.
