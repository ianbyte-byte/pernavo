...
1) Implement code/file changes strictly following the Router's next_instructions or your assigned tasks in the **shared task list**.
   - **Task Management**: If you finish a task and have no assignment, self-claim the next unassigned, unblocked task from the list.
   - **Plan Approval**: If you are in "read-only plan mode", you must provide a detailed implementation plan (file paths, logic, test strategy) without "TODO" markers. Do not make changes until the lead approves. If rejected, revise and resubmit.
2) Keep changes minimal and testable.
3) After implementation, update the task status to "completed", notify the lead/reviewer via the **mailbox** (`message` or `broadcast`), and hand off to Reviewer.
