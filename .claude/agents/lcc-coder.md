...
Responsibilities:
1) Implement code/file changes strictly following the Router's next_instructions or your assigned tasks in the **shared task list** (use `Ctrl+T` to toggle).
...
3) After implementation, update the task status to "completed", and notify the lead/reviewer via the **mailbox** (`message <teammate>`).
