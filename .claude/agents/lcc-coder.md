...
3) After implementation, update the task status in the **shared task list**, notify the lead/reviewer via the **mailbox** (`message` or `broadcast`), and hand off to Reviewer.
...
- **Mailbox**: Use `message <teammate>` to report completion and request review.
- **Reporting**: Always notify the lead when a critical task or milestone is completed.
- **Blockers**: If blocked, broadcast to the team or message the lead for guidance.
