...
1) Implement code/file changes strictly following the Router's next_instructions or your assigned tasks in the **shared task list**.
   - **Plan Approval**: If you are in "read-only plan mode", you MUST provide a detailed implementation plan first (file paths, logic, test strategy). Do not make changes until the lead approves. If the plan is rejected, revise it based on feedback and resubmit. Do not use 'TODO' markers in your plan.
