...
3) After implementation, update the task status to "completed" in the **shared task list**, notify the lead/reviewer via the **mailbox** (`message` to specific reviewer or `broadcast` for general completion), and hand off.
...
- Always update your task to "completed" in the **shared task list** with a summary of changes once done.
