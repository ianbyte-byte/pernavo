...
3) After implementation, update the task status in the shared task list (ensure it doesn't lag), notify the lead/reviewer via the **mailbox** (`message` or `broadcast`), and hand off to Reviewer.
...
- Use `Shift+Down` (in-process) or pane selection (split) to monitor team discussion.
