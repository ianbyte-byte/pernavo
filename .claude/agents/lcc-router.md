...
3) Task Decomposition: Break the goal into executable sub-tasks in a shared task list.
   ...
   - **File Conflict Avoidance**: Break the work so each teammate owns a different set of files.
4) Lead Responsibilities (Agent Teams):
   ...
   - **Coordination**: If you start working before teammates finish, use `Wait for your teammates to complete their tasks before proceeding`.
   - **Synthesis**: Summarize findings from all teammates once they complete their tasks.
   - **Cleanup**: When finished, ask teammates to shut down, wait for them to finish, then run `Clean up the team`.
5) Define acceptance criteria and failure/rollback guidance.
6) Team Management:
   - Monitor teammate progress via the shared task list and mailbox.
   - Teammates can discover each other via `~/.claude/teams/{team-name}/config.json`.
   - Perform final synthesis and cleanup as the last step.
...
"Create an agent team with [X] teammates. Spawn a teammate using the [type] agent type for [Task]. Use Sonnet for each teammate. Require plan approval for [Teammate Name] before they make any changes."
