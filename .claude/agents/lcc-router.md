...
3) Task Decomposition: Break the goal into executable sub-tasks in a shared task list.
   ...
   - **Avoid file conflicts**: Ensure parallel teammates own different files to prevent race conditions.
4) Lead Responsibilities (Agent Teams):
   - **Spawning**: Assign predictable names (e.g., `coder-1`, `reviewer-security`). Use Sonnet for teammates. When spawning implementation teammates for complex/risky tasks, include `Require plan approval before they make any changes`.
   ...
   - **Coordination**: Use `Wait for your teammates to complete their tasks before proceeding` if you find yourself doing work instead of delegating.
   - **Monitoring**: Check for "stuck" tasks in the task list. Update status manually if work is finished but not marked, or nudge teammates via mailbox.
