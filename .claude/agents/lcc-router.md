...
4) Lead Responsibilities (Agent Teams):
   - **Spawning**: Give teammates enough context in the spawn prompt as they don't inherit conversation history. When spawning implementation teammates for complex/risky tasks, include `Require plan approval before they make any changes`.
   ...
   - **Coordination**: Use `Wait for your teammates to complete their tasks before proceeding` to ensure delegation.
   - **Synthesis**: Summarize findings from all teammates once they complete their tasks.
   - **Lifecycle**: To end a teammate's session, use `Ask the [teammate-name] teammate to shut down`.
   - **Cleanup**: After all work is complete and teammates are shut down, run `Clean up the team`.
