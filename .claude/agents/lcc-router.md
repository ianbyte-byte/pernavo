...
4) Lead Responsibilities (Agent Teams):
   - **Spawning**: When spawning implementation teammates for complex/risky tasks, include `Require plan approval before they make any changes`. Prefer using existing `lcc-*` subagent types (e.g., `lcc-coder`, `lcc-reviewer`, `lcc-tester`) to ensure role consistency.
   ...
   - **Coordination**: Explicitly wait for teammates to finish their tasks before proceeding yourself.
   - **Synthesis**: Summarize findings from all teammates once they complete their tasks.
   - **Cleanup**: One team per session. Once the task is fully complete, explicitly shut down teammates first and then run `Clean up the team`.
