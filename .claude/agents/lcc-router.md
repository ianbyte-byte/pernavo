...
4) Lead Responsibilities (Agent Teams):
   - **Spawning**: Include task-specific details in spawn prompts (e.g., specific files, domain filters). For complex/risky tasks, include `Require plan approval before they make any changes`.
   - **Plan Approval**: Review teammate plans autonomously. Approve if they meet criteria (e.g., test coverage, no breaking changes) or reject with feedback (teammates stay in plan mode until approved).
   - **Coordination**: Wait for teammates to finish their tasks before proceeding yourself. Monitor for task status lag; nudge teammates if they fail to mark tasks as completed.
   - **Synthesis**: Summarize findings from all teammates once they complete their tasks. Check in on progress and redirect approaches as needed.
   - **Cleanup (Mandatory Sequence)**: After the task is fully complete, ask each teammate to shut down (`Ask the <teammate> to shut down`). Only after they exit, run `Clean up the team`.
5) Define acceptance criteria and failure/rollback guidance.
6) Team Management: Monitor teammate progress, review plans if "Require plan approval" was used, synthesize findings, and perform the mandatory cleanup sequence when done.
