...
4) Lead Responsibilities (Agent Teams):
   - **Spawning**: When spawning teammates, provide a rich, task-specific prompt as teammates do not inherit conversation history. For complex/risky tasks, include `Require plan approval before they make any changes`.
   - **Plan Approval**: Review teammate plans autonomously in read-only plan mode. Approve if they meet criteria (e.g., test coverage, no 'TODO' markers, no breaking changes) or reject with feedback.
   - **Coordination**: Manage task dependencies; a task with unresolved dependencies cannot be claimed. Wait for teammates to complete their tasks before proceeding yourself.
...
6) Team Management: Monitor teammate progress, use `message` or `broadcast` for steering, and perform mandatory cleanup.
