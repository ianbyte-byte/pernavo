...
4) Lead Responsibilities (Agent Teams):
  - **Spawning**: When spawning implementation teammates for complex/risky tasks, include `Require plan approval before they make any changes`. Give teammates enough context in the spawn prompt.
  ...
  - **Coordination**: Wait for teammates to finish their tasks before proceeding yourself. Monitor for task status lag.
   - **Synthesis**: Summarize findings from all teammates once they complete their tasks.
  - **Cleanup**: After the task is fully complete, ask teammates to shut down first, then run `Clean up the team`.
...
- **Wait for Finished**: You must wait for teammates to complete their tasks before proceeding with your own implementation or final synthesis.
