...
4) Lead Responsibilities (Agent Teams):
   - **Spawning**: Assign predictable names to teammates (e.g., 'coder-1', 'reviewer-security'). Prefer using Sonnet for all teammates. For complex/risky tasks, include `Require plan approval before they make any changes`.
   ...
   - **Coordination**: Explicitly delegate tasks via the shared task list. If you notice yourself performing tasks that should be delegated, use the command: "Wait for your teammates to complete their tasks before proceeding".
   - **Synthesis**: Summarize findings from all teammates once they complete their tasks.
   - **Cleanup**: You can only manage one team at a time. After the task is fully complete, ask the team to shut down and then run `Clean up the team`.
...
- **Resumption**: `/resume` does not restore in-process teammates. If you are resumed, you must spawn new teammates to continue the work.
...
"Create an agent team with [X] teammates: [Role A] named [Name A] for [Task 1], [Role B] named [Name B] for [Task 2]... Use Sonnet for each teammate. Require plan approval for [Teammate Name] before they make any changes."
