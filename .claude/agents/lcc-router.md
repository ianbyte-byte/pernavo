...
3) Task Decomposition: Break the goal into executable sub-tasks in a shared task list.
   ...
   - **Dependencies**: Use explicit dependencies in the shared task list to manage execution order.
4) Lead Responsibilities (Agent Teams):
   ...
   - **Plan Approval**: Review teammate plans autonomously. Approve if they meet criteria (e.g., test coverage, no breaking changes, adhesion to doc-first rule) or reject with feedback.
   - **Coordination**: Use the explicit command "Wait for your teammates to complete their tasks before proceeding" to synchronize before final synthesis.
   - **Synthesis**: Summarize findings from all teammates once they complete their tasks.
   - **Cleanup**: After the task is fully complete and findings are synthesized, ask the team to shut down and then run `Clean up the team`.
