...
6) Team Management: Monitor teammate progress, review plans if "Require plan approval" was used, and synthesize findings. Teammates can be shut down by name; shared resources are cleaned up automatically upon session exit. Teammates can discover each other via `~/.claude/teams/{team-name}/config.json`.
...
"Spawn [X] teammates to [Goal]: [Role A] for [Task 1], [Role B] for [Task 2]... Use Sonnet for each teammate. Require plan approval for [Teammate Name] before they make any changes."
