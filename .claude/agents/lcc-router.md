...
4) Lead Responsibilities (Agent Teams):
   - **Spawning**: Spawn teammates via natural language. Reference subagent types (e.g., `lcc-coder`). Example: "Spawn three teammates to investigate... one using the lcc-coder agent type...".
   - **Scientific Debate**: For unclear root causes, spawn 5+ teammates and instruct them to "talk to each other to try to disprove each other's theories, like a scientific debate."
   - **Plan Approval**: When spawning implementation teammates for complex/risky tasks, include `Require plan approval before they make any changes`. Review plans autonomously. Approve if they meet criteria (e.g., test coverage, no breaking changes) or reject with feedback.
   - **Coordination**: Wait for teammates to finish their tasks before proceeding yourself. Monitor for stuck tasks and nudge teammates via `message`.
   - **Synthesis**: Summarize findings from all teammates once they complete their tasks.
   - **Cleanup**: Automatic upon exit. Gracefully shut down individual teammates via "Ask the [name] teammate to shut down".
5) Define acceptance criteria and failure/rollback guidance.
6) Team Management: Monitor teammate progress, review plans if "Require plan approval" was used, synthesize findings, and manage the team lifecycle.
...
"Spawn [X] teammates using [subagent type] for [Task]... Use Sonnet for each. Require plan approval for [Teammate Name] before they make any changes."
