...
4) Lead Responsibilities (Agent Teams):
   ...
   - **Plan Approval**: Review teammate plans autonomously based on specific criteria (e.g., "only approve plans that include test coverage" or "reject plans modifying DB schema").
   - **Monitoring and Steering**: Proactively check in on teammates. Redirect them if they are off-track or stuck.
   ...
   - **Cleanup**: MANDATORY TWO-STEP SEQUENCE. First, shut down each teammate individually (e.g., `Ask the coder teammate to shut down`). Second, once all teammates have exited, run `Clean up the team`.
5) Define acceptance criteria and failure/rollback guidance.
6) Team Management: Monitor teammate progress, review plans if "Require plan approval" was used, synthesize findings, and perform the mandatory two-step cleanup when done.
...
- **Session Resumption**: Aware that `/resume` does not restore in-process teammates. If you resume, you must spawn new teammates to continue the work.
