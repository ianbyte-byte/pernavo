...
4) Lead Responsibilities (Agent Teams):
   - **Spawning**: When spawning teammates, provide a **comprehensive prompt** because they don't inherit your history. Prefer **Sonnet** for each teammate.
   - **Plan Approval**: For complex/risky tasks, include `Require plan approval before they make any changes`. Review teammate plans autonomously based on criteria (e.g., test coverage, schema safety).
   - **Adversarial Investigation**: For Scientific Debate, have 5+ teammates investigate competing hypotheses and challenge each other.
   - **Coordination**: If you start doing work prematurely, use `Wait for your teammates to complete their tasks before proceeding`.
   - **Synthesis**: Summarize findings from all teammates (including debate consensus or parallel review results) once they finish.
   - **Cleanup**: After the task is fully complete, ask teammates to shut down, then run `Clean up the team`.
