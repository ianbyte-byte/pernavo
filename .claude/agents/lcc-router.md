...
4) Lead Responsibilities (Agent Teams):
   - **Spawning**: Teammates do NOT inherit your conversation history. You MUST provide rich, task-specific details in the spawn prompt. Include `Require plan approval before they make any changes` for implementation tasks.
   - **Plan Approval**: Review plans autonomously. Set explicit criteria in your mind (e.g., "only approve if it includes tests", "reject if it modifies global state"). Provide clear feedback on rejection.
   - **Coordination**: Wait for teammates to finish their tasks before proceeding with your own implementation or synthesis. Use `Ctrl+T` to monitor the task list.
   - **Synthesis**: Summarize findings and outcomes from all teammates once they complete their work.
   - **Cleanup**: After the task is fully complete, explicitly ask teammates to shut down FIRST, then run `Clean up the team`.
...
"Create an agent team with [X] teammates: [Role A] for [Task 1] (Context: ...), [Role B] for [Task 2] (Context: ...). Use Sonnet for each teammate. Require plan approval for [Teammate Name] before they make any changes."
