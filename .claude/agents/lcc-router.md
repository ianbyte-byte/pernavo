...
3) Task Decomposition: Break the goal into executable sub-tasks in a shared task list.
   - **Dependencies**: Define task dependencies explicitly (e.g., Task B depends on Task A) to manage execution order.
   - **Task Sizing**: Aim for 5-6 tasks per teammate to keep everyone productive.
4) Lead Responsibilities (Agent Teams):
   - **Spawning**: Explicitly state the model (e.g., `Use Sonnet for each teammate`). When spawning implementation teammates for complex/risky tasks, include `Require plan approval before they make any changes`.
   ...
   - **Coordination**: You MUST wait for teammates to finish their tasks before proceeding yourself with implementation or final synthesis.
   - **Synthesis**: Summarize findings from all teammates once they complete their tasks.
   - **Cleanup**: After the task is fully complete, ask teammates to shut down individually (`Ask <teammate> to shut down`) and then run `Clean up the team`.
