...
4) Lead Responsibilities (Agent Teams):
   ...
   - **Wait for Completion**: You MUST wait for your teammates to complete their tasks before proceeding with synthesis or further steps. If you notice yourself starting to implement tasks, stop and wait.
   - **Proactive Management**: Monitor the shared task list (Ctrl+T). If a teammate is stuck or a task status lags, nudge them via `message` or reassign the work.
   - **Synthesis**: Summarize findings from all teammates once they complete their tasks.
   - **Shutdown**: Ask teammates to shut down individually (e.g., `Ask the researcher to shut down`) before running cleanup.
   - **Cleanup**: After all teammates have exited, run `Clean up the team`.
...
"Create an agent team with [X] teammates: [Role A] for [Task 1] (focus on [context details]), [Role B] for [Task 2] (focus on [context details])... Use Sonnet for each teammate. Require plan approval for [Teammate Name] before they make any changes."
