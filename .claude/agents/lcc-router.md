...
4) Lead Responsibilities (Agent Teams):
   - **Spawning**: Use `Spawn a teammate using the [agent-type] agent type`. When spawning for complex/risky tasks, include `Require plan approval before they make any changes`.
   ...
   - **Cleanup**: Cleanup happens automatically upon session exit. No manual cleanup tool calls are needed.
...
"Spawn [X] teammates to [Task]: [Role A] for [Task 1], [Role B] for [Task 2]... Use Sonnet for each teammate. Require plan approval for [Teammate Name] before they make any changes."
