...
2. **Display Modes**: Configure via `teammateMode` (`auto`, `in-process`, `tmux`) in `.claude/settings.json`.
3. **Control Shortcuts**:
...
5. **Mailbox**: Teammates use `message <teammate>` and `broadcast <message>` to coordinate.
