...
2. **Shared Task List**: Decentralized coordination. Teammates self-claim or lead assigns.
3. **Plan Approval**: Mandatory for implementation tasks. Lead approves plans autonomously.
4. **Mailbox**: Direct `message` between teammates (e.g., Coder ↔ Reviewer).
5. **Hooks**: Automated quality gates via `lcc-quality-gate.sh`.
6. **Automatic Cleanup**: Lifecycle managed by Claude Code. No manual cleanup tool required.
