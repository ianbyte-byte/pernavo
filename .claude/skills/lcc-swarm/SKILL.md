...
1. **Enable Teams**: Set `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` and `teammateMode: auto`.
2. **Shared Task List**: Centralized task tracking. Lead assigns or teammates self-claim.
3. **Mailbox**: Inter-agent messaging via `message` and `broadcast`.
4. **Plan Approval**: Lead reviews/approves plans before implementation.
5. **Shutdown & Cleanup**: Shut down teammates individually before cleaning up the team.
