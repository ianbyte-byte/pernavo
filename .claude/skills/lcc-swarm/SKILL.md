...
2. **Shared Task List**: Lead manages tasks (Ctrl+T); teammates claim and complete.
3. **Mailbox**: Teammates use `message` and `broadcast` to coordinate.
4. **Shortcuts**: Use `Shift+Down` to cycle through teammates.
5. **Plan Approval**: Mandatory for complex tasks (`Require plan approval`).
