...
- **Shared task list**: Use it to assign and track work. Teammates can self-claim tasks. Aim for 3-5 teammates for most workflows, with 5-6 tasks per teammate to maximize productivity.
- **Plan Approval**: For complex or risky tasks (e.g., refactors), the lead should spawn teammates with `Require plan approval before they make any changes`. The lead reviews and approves/rejects plans autonomously (e.g., rejecting plans that lack test coverage or contain 'TODO' markers).
- **Teammate Mode**: Configure `teammateMode: "auto"` in `.claude/settings.json`.
- **Shortcuts (In-process mode)**:
  - `Shift+Down`: Cycle through teammates (wraps to lead).
...
- **Communication**:
  ...
  - `broadcast <message>`: Send to all teammates (use sparingly as costs scale with team size).
- **Shutdown and Cleanup**: The lead must wait for teammates to finish, request them to shut down, and then run `Clean up the team` to remove shared resources.
- **Parallel patterns**:
...
- **Avoid File Conflicts**: Break the work so each teammate owns a different set of files.
