...
- **Teammates**: Independent agents with their own context windows. They **do not inherit** the lead's conversation history; provide all necessary context in the spawn prompt.
- **Display modes**:
  - `in-process`: Default. All teammates run in one terminal. Use `Shift+Down` to cycle.
  - `split-pane`: Requires `tmux` or `iTerm2`. Set via `"teammateMode": "tmux"` in `.claude/settings.json`.
- **Shared task list**: Decentralized coordination. Teammates can self-claim unassigned, unblocked tasks. Tasks can have dependencies; a task is blocked until its dependencies are complete. File locking prevents race conditions. Aim for 5-6 tasks per teammate.
- **Plan Approval**: For complex/risky tasks, spawn with `Require plan approval before they make any changes`. The lead reviews plans in "read-only plan mode" and approves/rejects autonomously.
- **Communication**:
  - `message <teammate>`: Direct message to a specific teammate (e.g., Coder to Reviewer).
  - `broadcast <message>`: Send to all teammates.
- **Cleanup**: The lead must shut down all teammates (`Ask <teammate> to shut down`) and then run `Clean up the team`.
- **Limitations**: `/resume` and `/rewind` do not restore in-process teammates. If resuming, tell the lead to spawn new teammates.
- **Parallel patterns**:
...
- **TaskCreated**: Fires when a task is added to the list. Used to enforce naming/description standards.
- **TaskCompleted**: Fires when a task is closed. Used to verify completion criteria and handoff reports.
