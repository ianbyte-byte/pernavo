...
- **Team lead**: The main agent session. Responsible for spawning the team, approving plans, and final synthesis. The lead must wait for teammates to complete their tasks before proceeding.
- **Teammates**: Independent agents with their own context windows. They can discover other team members via `~/.claude/teams/{team-name}/config.json`.
- **Shared task list**: Use it to assign and track work. Teammates can self-claim tasks. Aim for 5-6 tasks per teammate to maximize productivity. Tasks have three states: pending, in progress, and completed.
- **Plan Approval**: For complex or risky tasks (e.g., refactors), the lead should spawn teammates with `Require plan approval before they make any changes`. The lead reviews and approves/rejects plans autonomously (reject if 'TODO' or lack of tests).
- **Shortcuts & Display**:
  - `Shift+Down`: Cycle through teammates (in-process mode).
  - `Ctrl+T`: Toggle task list.
  - `Enter`: View teammate session; `Escape`: Interrupt.
- **Communication**:
...
- **Limitations**: Agent teams are experimental. `/resume` does not restore in-process teammates. Task status can lag; the lead should nudge teammates if needed.
- **Cleanup**: Once the task is complete, the lead must explicitly ask teammates to shut down and then run `Clean up the team` to remove shared resources.
- **Parallel patterns**:
