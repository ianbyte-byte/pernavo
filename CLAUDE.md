...
- **Display Modes**:
  - `in-process`: Cycle with `Shift+Down`, toggle task list with `Ctrl+T`.
  - `split-pane`: Requires tmux/iTerm2. Configure via `teammateMode` in `settings.json`.
...
- **Cleanup Sequence (Mandatory)**:
  1. **Shutdown**: The lead must shut down each teammate individually (`Ask the <teammate> to shut down`).
  2. **Cleanup**: Run `Clean up the team` via the lead only after all teammates have exited.
- **Parallel patterns**:
