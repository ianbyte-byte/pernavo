...
- **Shared task list**: Use it to assign and track work. Teammates can self-claim tasks. Aim for 5-6 tasks per teammate. Toggle with `Ctrl+T`.
...
- **Controls**: `Shift+Down` (cycle teammates), `Escape` (interrupt current turn).
- **Communication**:
...
- **Cleanup**: Once the task is complete, the lead must shut down all teammates individually (e.g., `Ask the coder teammate to shut down`) and then run `Clean up the team` to remove shared resources.
- **Parallel patterns**:
