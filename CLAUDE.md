...
## 3) Handoff protocol (mandatory - V2.2 Schema)
...
- **Discovery**: Teammates should discover other members by reading `~/.claude/teams/{team-name}/config.json`.
- **Shared task list**: Use it to assign and track work. Teammates can self-claim tasks.
  - **Dependencies**: Use task dependencies for sequential work. Blocked tasks cannot be claimed.
...
- **Cleanup**: Once the task is complete, the lead must FIRST shut down all teammates, and then run `Clean up the team`.
- **Parallel patterns**:
