...
### Agent Teams
Enabled via `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`. Use for parallel work and complex coordination.
...
PYTHONPATH=src python -m chung_agent_swarm.cli session-config validate

# Validate handoff envelope (stdin)
PYTHONPATH=src python -m chung_agent_swarm.cli handoff validate
```
