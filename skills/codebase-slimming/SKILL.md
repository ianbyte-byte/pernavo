...
圈复杂度与生产规模必须同时约束：不得靠集中分支、嵌套或布尔万能函数换行数；也不得为降低单函数复杂度、却无真实变化轴地拆文件。体积下降而复杂度上升，不算瘦身成功。行数下降不能证明 bug 更少。
...
体积与圈复杂度是双约束，不是二选一。禁止把“少写代码”理解成跳过基线、删测试，或把分支压进更难穷举的控制流。
...
`scripts/quality_evidence.py` 与 `skills/change-review/scripts/sonarqube_local.py`；不得把 MCP 配置存在、镜像存在或
容器运行误报成当前 Agent 会话已经发现并调用了质量工具。
只读查询已有 SonarQube 项目、质量门和问题时，组合使用
[change-review](../change-review/SKILL.md)，不要在瘦身流程中重复实现查询逻辑。
...
**COMPLETED 条件（同时满足）：** 约定范围完成；构建与测试/QA 通过；API 与 DB 兼容；正式目标达成或偏差可解释；无高尔夫、未删测试/有价值注释、未引入更危险通用抽象；体积下降未抬高圈复杂度；护栏落地；人工审核通过。

## 一句话规则

> 代码库瘦身不是把代码压缩得更短，而是在行为可验证的前提下，逐步移除重复、危险和失控的实现，使系统更小、更清晰、路径更少；体积下降不得以圈复杂度上升为代价。
