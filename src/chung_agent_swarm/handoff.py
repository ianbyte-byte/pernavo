...
def parse_handoff_dict(obj: Mapping[str, Any]) -> HandoffEnvelope:
    ...
    if isinstance(summary, str):
        ...
        # Validate enhanced summary fields if present
        for field in ["progress", "remaining", "risks", "changes"]:
            val = summary.get(field)
            if val is not None and not isinstance(val, str):
                raise HandoffValidationError(f'Summary field "{field}" must be a string.')
    else:
