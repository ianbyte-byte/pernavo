...
- **Display Modes**: `in-process` (default) or `split-panes` (requires tmux/iTerm2). Set `teammateMode` in settings.
- **Cleanup**: Team cleanup happens automatically upon session exit. The `TeamCreate` and `TeamDelete` tools are deprecated. Graceful shutdown of teammates is recommended.
