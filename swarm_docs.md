...
V2.2 leverages native Claude Code **Agent Teams** (v2.1.32+) with enhanced orchestration:
...
- **Plan Approval**: Use `Require plan approval` for complex tasks. The lead reviews and approves/rejects plans in read-only mode.
- **Task Sizing**: Aim for 5-6 tasks per teammate to maximize productivity.

### 4.2 Patterns
- **Scientific Debate**: 5+ teammates investigating competing hypotheses. Mechanism: "try to disprove each other's theories".
...
- **Mailbox**: inter-agent messaging via `message <teammate>` (direct by name) and `broadcast` (team-wide).
- **Cleanup**: The lead must shut down teammates by name first, then run `Clean up the team`.
