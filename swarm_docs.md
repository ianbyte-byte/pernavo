...
Each handoff must include a JSON object (V2.2 Schema):
...
```json
{
  ...
  "context": { "risk_level": "low", "platform_api_needed": false }
}
```
...
- `acceptance_criteria` is mandatory for complex handoffs.
...
- **Plan Approval**: Use `Require plan approval` for complex tasks. The lead reviews and approves/rejects plans before implementation begins based on criteria (e.g., test coverage).
- **Task Sizing**: Aim for 5-6 tasks per teammate to maximize productivity.
- **Wait for Completion**: Use "Wait for your teammates to complete their tasks before proceeding" to synchronize.
...
- **Shutdown & Cleanup**: The lead must explicitly ask teammates to shut down, then run `Clean up the team` after completion.
- **Display Modes**: Supports "auto", "tmux", and "in-process" modes.
