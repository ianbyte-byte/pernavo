...
- **Display Mode**: Set `teammateMode: "auto"` in `settings.json` to favor split-panes (tmux/iTerm2) when available.

### 4.2 Patterns
- **Scientific Debate**: 5+ teammates investigating competing hypotheses and actively trying to disprove each other to reach a robust consensus.
- **Parallel Review**: Assign reviewers with distinct lenses (Security, Performance, Test Coverage) to PRs or modules.
- **Cross-layer coordination**: Separate teammates for frontend, backend, and testing working in parallel from the same shared task list.

### 4.3 Coordination
- **Shared Task List**: Decentralized task tracking and self-claiming.
- **Mailbox**: Inter-agent messaging via `message <teammate>` (direct) and `broadcast` (team-wide).
- **Cleanup**: The lead must wait for completion (`Wait for your teammates...`), perform a final synthesis, shut down teammates, and run `Clean up the team`.

### 4.4 Automated Quality Gates
- `TaskCreated`: Rejects tasks with subjects < 10 characters or containing "TODO".
- `TaskCompleted`: Validates that a handoff report or summary exists in the transcript and rejects "TODO" in subjects.
