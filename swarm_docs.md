...
- **Control**: Use `Shift+Down` to cycle, `Ctrl+T` for tasks, `Enter` to view, and `Escape` to interrupt.
- **Cleanup**: The lead must shut down teammates and run `Clean up the team` after completion. Only one team per session.
