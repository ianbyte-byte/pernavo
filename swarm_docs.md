...
V2.2 leverages native Claude Code **Agent Teams** with advanced orchestration and steering:
...
- **Wait for Completion**: Use `Wait for your teammates to complete their tasks before proceeding` to ensure delegation over-execution.
...
- **Discovery**: Teammates can read `~/.claude/teams/{team-name}/config.json` to discover peer session IDs.
